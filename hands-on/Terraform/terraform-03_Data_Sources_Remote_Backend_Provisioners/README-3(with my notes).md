...
- Go to the `main.tf` file make the changes. (in s3-backend file)
...
- Go to the `terraform-aws` directoy and run the commands below. First try to terraform apply command. (it must be s3-backend file)
...
resource "aws_instance" "instance" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("~/oliver.pem")  /*type your path (~/Documents/KEY/felix.pem*/ 
  }
